...
Este endpoint `devuelve` el asunto y el zip generado en formato **Base64**, datos que normalmente se envían al cliente por correo electrónico.
Está diseñado para escenarios en los que el envío automático del correo ha sido deshabilitado `send_email = false` al momento de validar la factura, permitiendo así que el integrador gestione manualmente el envío del correo al cliente final.
El archivo adjunto corresponde al archivo zip que contiene el pdf de la factura y el attached document que se enviaría en el correo oficial.
...
[Sección titulada «Variables de Ruta, Path Variables»](https://developers.factus.com.co/facturas/obtener-contenido-de-correo#variables-de-ruta-path-variables)
