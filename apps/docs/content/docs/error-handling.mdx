...
try {
  const bill = await factus.bills.get("NO-EXISTE");
} catch (error) {
...
<Callout type="warn">
  Factus tiene un límite de **80 solicitudes por minuto**. Si se excede, la API
  responde `429 Too Many Requests` y puede incluir headers como
